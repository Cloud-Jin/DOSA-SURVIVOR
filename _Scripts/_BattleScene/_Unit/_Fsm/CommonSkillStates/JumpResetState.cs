...
{
    public class JumpResetState : SkillState, IState
    {
        private int count;              // 스킬 시전횟수
        private bool isAction;
        private float durationTime;
        private Vector3 EndPonsition;
        private Vector3 EndPonsitionReset;
        public JumpResetState(UnitBase unit, SkillAI data, StateMachine stateMachine)
        {
            caster = unit;
            this.stateMachine = stateMachine;
            this.data = data;
            EndPonsitionReset = unit.transform.position;
        ...
        IEnumerator JumpDown()
        {
            ...
            {
                EndPonsition = EndPonsitionReset;
                // EndPonsition = MyMath.GetBossSpawnPosition(PlayerManager.Instance.PlayerRigidBody2D.transform, Map.Infinite);
            }
