...
namespace ProjectM.Battle
{
    public class ChallengeSpawner : Spawner
    {
        ...
        protected override void Init()
        {
            ...
            var value = challengeBattleManager?.GetPenaltyValue(ChallengePenalty.Crack)??0;
            if (value == 0)
            {
            ...
            var mapSkillValue = challengeBattleManager?.GetPenaltyValue(ChallengePenalty.MapSkill)??0;
            if (mapSkillValue > 0)
            {
                var monsterDummy = TableDataManager.Instance.data.Monster.Single(t => t.Index == mapSkillValue);
                var _dummy = ResourcesManager.Instance.Instantiate("PenaltyDummy");
                var dummy = _dummy.GetOrAddComponent<EnemyNormal>();
                dummy.SetType(UnitType.Enemy, EnemyType.Summon);
                dummy.Init(monsterDummy);
                dummy.SetSpawnData(monsterEnhanceBaseRatio, monsterHPIncrease, monsterAttackIncrease,
                    monsterExpMultiple);
                dummy.transform.SetParent(_player.transform);
            }
        }
        ...
        void BossSpawn(int j)
        {
            ...
            if (bm.GetBoss.Count == 1)
            {
                spawnPosition.Add(position);
                
                foreach (var t in spawnPosition)
                {
                    // 보스마크
                    var _mark = ResourcesManager.Instance.Instantiate("Effect_BossStart_00");
                    mark.Add(_mark);
                    _mark.transform.position = t;
                }
            ...
            else
            {
                if (map == Map.Vertical)
                {
                    spawnPosition.Add(position + new Vector3(0, -1, 0));
                    spawnPosition.Add(position + new Vector3(0, 1, 0));
                }
                else
                {
                    spawnPosition.Add(position + new Vector3(-2, 0, 0));
                    spawnPosition.Add(position + new Vector3(2, 0, 0));
                }
            ...
            seq.AppendCallback(() =>
            {
                ...
                for (int i = 0; i < bm.GetBoss.Count; i++)
                {
                    ...
                    enemyBoss.transform.position = spawnPosition[i];
                    ...
                    enemyBoss.GetOrAddComponent<EnemyBoss>().SetPenalty();
