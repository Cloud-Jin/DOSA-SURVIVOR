...
{
    public class TutorialStageSpawner : Spawner
    {
        // # 균열
        ...
        public void CrackSpawn(uint i)
        {
            ...
            if (TutorialManager.Instance.IsTutorialBattle)
            {
                ...
                for (int j = 0; j < 100; j++)
                {
                    var pos = _player.transform.position + MyMath.RandomCirclePoint(2.5f);
        ...
        private void EnemySpawn(int remainCount)
        {
            // 시간당 n 마리 생성
